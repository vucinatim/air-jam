...
const BoxesCore = ({ className, ...rest }: { className?: string }) => {
  ...
  useEffect(() => {
    ...
    const spawnRipple = () => {
      ...
      for (let d = 0; d <= RIPPLE_RADIUS; d++) {
        ...
        for (let r = Math.max(0, centerRow - RIPPLE_RADIUS); r <= Math.min(ROWS - 1, centerRow + RIPPLE_RADIUS); r++) {
          for (let c = Math.max(0, centerCol - RIPPLE_RADIUS); c <= Math.min(COLS - 1, centerCol + RIPPLE_RADIUS); c++) {
  ...
  return (
    <div
      ...
      className={cn(
        "absolute -top-1/4 left-1/4 z-0 flex p-4",
        className,
      )}
      {...rest}
    >
        ...
        >
          {Array.from({ length: COLS }, (_, j) => (
            <div
              key={j}
              data-cell=""
              className="relative h-8 w-16 border-t border-r border-border/20 transition-[background-color] duration-1000 ease-out"
            >
              {j % 2 === 0 && i % 2 === 0 ? (
                <svg
