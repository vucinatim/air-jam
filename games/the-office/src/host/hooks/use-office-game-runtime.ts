...
import type { GameInput } from "../../game/contracts/input";
import {
  BREAKROOM_LOCATIONS,
  LOCATIONS,
  STAT_CONSTANTS,
...
import {
  ...
  PLAYER_SPEED,
  SPAWN_POSITIONS,
} from "../../game/engine/office-layout";
import {
...
import { useOfficeSounds } from "./use-office-sounds";
...
export function useOfficeGameRuntime({
  ...
  const handleTaskComplete = useCallback(
    (playerId: string, matchElapsedMs: number): boolean => {
  ...
  const updateGame = useCallback(
    ...
    [handleTaskComplete, handleTaskStart, playNewOrder, playOrderTimeout],
  );
...
export function useOfficeActiveMatchClock(): {
  ...
  return {
    advanceElapsedMs,
    elapsedMsRef,
    timeRemainingMs: matchPhase === "lobby" ? gameDurationMs : timeRemainingMs,
  };
}
