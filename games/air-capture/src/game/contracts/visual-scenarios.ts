...
type AirCaptureVisualContext = VisualScenarioContext<typeof agentContract>;

const prepareLobbyState = async (
  context: AirCaptureVisualContext,
): Promise<void> => {
...
const prepareEndedState = async (
  ...
  await context.agent.invoke("player:force_end_match", {
    winner: "solaris",
    finalScores: {
      solaris: 1,
      nebulon: 0,
...
export const visualHarness = defineVisualHarness({
  ...
  scenarios: [
    ...
    },
    {
      id: "ended",
      description:
        "Ended surface after a deterministic semantic agent action finalizes the match with Solaris winning 1:0.",
      run: async (context) => {
