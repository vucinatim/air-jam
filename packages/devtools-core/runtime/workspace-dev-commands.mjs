...
export const runWorkspaceArcadeDevCommand = async ({
  ...
  const arcadeTopologies = createRepoWorkspaceArcadeTopologies({
    ...
    controllerPlatformOrigin: arcadeOrigins.publicPlatformOrigin,
    publicPlatformOrigin: arcadeOrigins.publicPlatformOrigin,
    embeddedHostRuntimeUrl: arcadeOrigins.hostGameOrigin,
    embeddedControllerRuntimeUrl: `${arcadeOrigins.publicGameOrigin}/controller`,
    secure,
  });
  const platformEnv = {
    AIR_JAM_DEV_PROXY_BACKEND_URL: BACKEND_PROXY_URL,
    NEXT_PUBLIC_AIR_JAM_PLATFORM_HOST_TOPOLOGY: serializeResolvedTopology(
      arcadeTopologies.platformHost,
    ),
    NEXT_PUBLIC_AIR_JAM_PLATFORM_CONTROLLER_TOPOLOGY: serializeResolvedTopology(
      arcadeTopologies.platformController,
    ),
