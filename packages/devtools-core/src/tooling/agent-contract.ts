...
if (!operation || (!contractPath && !configPath)) {
  throw new Error(
    "Missing required --operation and (--config or --contract) input for Air Jam agent contract helper.",
  );
}

const resolveContractFromModule = async (
  nextContractPath: string,
): Promise<AirJamAgentContract | null> => {
  const contractModule = (await import(
    pathToFileURL(nextContractPath).toString()
  )) as {
    default?: AirJamAgentContract;
    agentContract?: AirJamAgentContract;
  };

  return contractModule.agentContract ?? contractModule.default ?? null;
};
...
if (!resolvedContract) {
  ...
  throw new Error(
    configPath
      ? `Air Jam config "${configPath}" does not export a valid agent contract.`
      : `Agent contract module "${contractPath}" does not export agentContract.`,
  );
}

if (operation === "inspect") {
  const snapshotStoreDomains = listAirJamAgentStoreDomains(resolvedContract);
  process.stdout.write(
    `${JSON.stringify(
      {
        hasContract: true,
