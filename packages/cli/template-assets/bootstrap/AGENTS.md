...
This file and the project-local `skills/` directory are bootstrap-owned: once
the project is created, they belong to the project and Air Jam updates never
overwrite them. Canonical framework references live under `docs/airjam/` and
may be refreshed independently with `airjam ai-pack update`.
...
1. `docs/airjam/agent-gold-path.md`
2. `docs/airjam/docs-index.md`
3. `skills/index.md`
...
If you only need the shortest correct operational path, start with `docs/airjam/agent-gold-path.md` before reading the broader index.
...
These are project-owned local workflow modules.

They teach how to work in this repo and are never overwritten by Air Jam
guidance updates.
...
## Air Jam Machine-Operation Rule

Use the Air Jam MCP or the canonical `airjam` CLI first for Air Jam-native
workflows. MCP is the normal lane for MCP-capable clients; the CLI is the
equivalent terminal-agent lane and is fully discoverable through `--help`.

Use these machine surfaces to:
...
MCP-capable clients start with:

1. `pnpm exec airjam mcp doctor --dir . --json`
2. `pnpm exec airjam mcp init --dir . --force` only if you need to repair or regenerate the project-local MCP config

Terminal agents can operate the semantic lifecycle directly with:

1. `pnpm exec airjam status --dir .`
2. `pnpm exec airjam dev start --dir .`
3. `pnpm exec airjam session open --dir .`
4. `pnpm exec airjam session read <session-id> --dir .`
5. `pnpm exec airjam session invoke <session-id> <action-id> --payload '{}'`
6. `pnpm exec airjam session close <session-id> --dir .`
7. `pnpm exec airjam dev stop --dir .`

The local lifecycle commands above return JSON documents. Use ad hoc shell
logic only when neither the Air Jam MCP nor the `airjam` CLI exposes the needed
operation.
