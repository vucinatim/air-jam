...
export const registerRealtimeHandlers = (
  ...
  socket.on("host:system", (payload) => {
    ...
    if (!parsed.success) {
      logRealtimeEvent(
        ...
        "Rejected host system command with invalid payload",
        {
    ...
    if (!isHostAuthorizedForRoom(roomId)) {
      logRealtimeEvent(
        "warn",
        AIRJAM_DEV_LOG_EVENTS.host.systemRejected,
        "Rejected host system command because socket is unauthorized",
        {
          roomId,
          command,
          reason: "unauthorized",
    ...
    if (!session) {
      logRealtimeEvent(
        "warn",
        AIRJAM_DEV_LOG_EVENTS.host.systemRejected,
        "Rejected host system command because room was not found",
        {
          roomId,
          command,
  ...
  socket.on("host:state", (payload: ControllerStateMessage) => {
    ...
    if (!result.success) {
      logRealtimeEvent(
        "warn",
        AIRJAM_DEV_LOG_EVENTS.host.stateRejected,
        "Rejected host state update with invalid payload",
        {
          reason: "invalid_payload",
    ...
    if (!isHostAuthorizedForRoom(roomId)) {
      logRealtimeEvent(
        "warn",
        AIRJAM_DEV_LOG_EVENTS.host.stateRejected,
        "Rejected host state update because socket is unauthorized",
        {
          roomId,
          reason: "unauthorized",
        },
    ...
    if (!session) {
      logRealtimeEvent(
        "warn",
        AIRJAM_DEV_LOG_EVENTS.host.stateRejected,
        "Rejected host state update because room was not found",
        {
          roomId,
          reason: "room_not_found",
        },
    ...
    hostStateBroadcastSummary.record({
      ...
      data: {
        ...
        orientationChanged:
          previousOrientation !== session.controllerOrientation,
        latestStateVersion: broadcastPayload.state.stateVersion ?? 0,
      },
      metrics: {
        broadcastCount: 1,
  ...
  socket.on("host:signal", (payload: SignalPayload) => {
    ...
    if (!roomId) {
      logRealtimeEvent(
        "warn",
        AIRJAM_DEV_LOG_EVENTS.host.signalRejected,
        "Rejected host signal because socket is not bound to a room",
        {
    ...
    if (!session) {
      logRealtimeEvent(
        "warn",
        AIRJAM_DEV_LOG_EVENTS.host.signalRejected,
        "Rejected host signal because room was not found",
        {
          roomId,
          reason: "room_not_found",
          signalType: payload.type,
    ...
    if (payload.targetId) {
      ...
      logRealtimeEvent(
        "warn",
        AIRJAM_DEV_LOG_EVENTS.host.signalRejected,
        "Rejected host signal because target controller was not found",
        {
          roomId,
  ...
  socket.on("host:play_sound", (payload: PlaySoundEventPayload) => {
    ...
    if (!isHostAuthorizedForRoom(roomId)) {
      logRealtimeEvent(
        "warn",
        AIRJAM_DEV_LOG_EVENTS.host.playSoundRejected,
        "Rejected host sound playback because socket is unauthorized",
        {
          roomId,
          soundId,
          reason: "unauthorized",
    ...
    if (!session) {
      logRealtimeEvent(
        "warn",
        AIRJAM_DEV_LOG_EVENTS.host.playSoundRejected,
        "Rejected host sound playback because room was not found",
        {
          roomId,
          soundId,
          reason: "room_not_found",
    ...
    if (targetControllerId) {
      ...
      logRealtimeEvent(
        "warn",
        AIRJAM_DEV_LOG_EVENTS.host.playSoundRejected,
        "Rejected host targeted sound because controller was not found",
        {
          roomId,
          targetControllerId,
          soundId,
  ...
  socket.on("controller:play_sound", (payload: PlaySoundEventPayload) => {
    ...
    if (!isControllerAuthorizedForRoom(roomId)) {
      logRealtimeEvent(
        "warn",
        AIRJAM_DEV_LOG_EVENTS.controller.playSoundRejected,
        "Rejected controller sound playback because socket is unauthorized",
        {
          roomId,
          soundId,
          reason: "unauthorized",
    ...
    if (!hasControllerPrivilegeForRoom(roomId, "play_sound")) {
      logRealtimeEvent(
        "warn",
        AIRJAM_DEV_LOG_EVENTS.controller.playSoundRejected,
        "Rejected controller sound playback because privileged capability is missing",
        {
          roomId,
          soundId,
    ...
    if (!session) {
      logRealtimeEvent(
        "warn",
        AIRJAM_DEV_LOG_EVENTS.controller.playSoundRejected,
        "Rejected controller sound playback because room was not found",
        {
          roomId,
          soundId,
          reason: "room_not_found",
  ...
  socket.on("host:state_sync", (payload: unknown) => {
    ...
    if (!parsed.success) {
      logRealtimeEvent(
        "warn",
        AIRJAM_DEV_LOG_EVENTS.host.stateSyncRejected,
        "Rejected host state sync with invalid payload",
        {
          reason: "invalid_payload",
          issues: parsed.error.issues,
        },
    ...
    if (!session) {
      logRealtimeEvent(
        "warn",
        AIRJAM_DEV_LOG_EVENTS.host.stateSyncRejected,
        "Rejected host state sync because room was not found",
        {
          roomId,
          storeDomain,
          reason: "room_not_found",
    ...
    if (
      ...
      logRealtimeEvent(
        "warn",
        AIRJAM_DEV_LOG_EVENTS.host.stateSyncRejected,
        "Rejected host state sync because socket is unauthorized",
        {
          roomId,
          storeDomain,
          reason: "unauthorized",
