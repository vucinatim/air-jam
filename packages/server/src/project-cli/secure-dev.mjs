  ...
  if (!commandExists("mkcert", ["-help"])) {
    throw new Error(
      [
        ...
        "Install it once, then rerun `air-jam-server secure:init`.",
  ...
  if (!commandExists("cloudflared")) {
    throw new Error(
      [
        ...
        "Install it once, then rerun `air-jam-server secure:init --mode=tunnel ...`.",
      ].join("\n"),
    );
  }
};
...
export const initializeSecureDev = ({
  ...
  runtimeEnv = loadCreateAirJamRuntimeEnv({
    env,
    boundary: "air-jam-server.secure-init",
  }),
...
export const loadSecureDevState = ({
  ...
  const runtimeEnv = loadCreateAirJamRuntimeEnv({
    env,
    boundary: "air-jam-server.secure-state",
  });
  const paths = getSecurePaths(cwd);
  const state = readSecureDevState(paths.secureDevStateFile);
  if (!state) {
    throw new Error(
      "Missing .airjam/secure-dev.json. Run `air-jam-server secure:init` first.",
    );
  }
  ...
  if (mode === SECURE_MODE_LOCAL) {
    ...
    if ((state.lanIp ?? null) !== (currentLanIp ?? null)) {
      throw new Error(
        "Your LAN IPv4 address changed since secure setup was created. Run `air-jam-server secure:init` again.",
      );
    }
  }
  ...
  if (mode === SECURE_MODE_TUNNEL && !tunnelName) {
    throw new Error(
      "Missing CLOUDFLARE_TUNNEL_NAME for tunnel secure mode. Run `air-jam-server secure:init --mode=tunnel ...` again.",
    );
  }
...
export const runSecureInitCli = async ({
  ...
  if (hasFlag(argv, "--help") || hasFlag(argv, "-h")) {
    console.log(
      "Usage: air-jam-server secure:init [--mode=local|tunnel] [--hostname <dev.example.com>] [--tunnel <tunnel-name>]",
    );
    console.log("");
    console.log("Modes:");
    console.log(
