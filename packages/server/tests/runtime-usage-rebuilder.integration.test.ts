...
describe("runtime usage rebuilder", () => {
  maybeIt(
    "rebuilds session projections idempotently from the raw ledger",
    async () => {
      const runtimeDb = getRuntimeDb();
      ...
      const [
        ...
        runtimeDb
          ...
          .where(
            eq(runtimeUsageGameSegments.runtimeSessionId, runtimeSessionId),
          ),
        runtimeDb
          .select()
        ...
        runtimeDb
          ...
          .where(
            eq(
              runtimeUsageGameSessionMetrics.runtimeSessionId,
              runtimeSessionId,
            ),
          ),
        runtimeDb
  ...
  maybeIt(
    "removes stale session and daily metrics when replayed ledger no longer produces gameplay",
    async () => {
      const runtimeDb = getRuntimeDb();
      const runtimeSessionId = `runtime_rebuild_${crypto.randomUUID()}`;
      await insertLedgerFixture(runtimeSessionId);
