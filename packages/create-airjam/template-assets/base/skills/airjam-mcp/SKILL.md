...
If a clean-slate project becomes non-trivial, add a small `src/game/contracts/agent.ts` surface and wire it through `src/airjam.config.ts` `game.machine.agent` before the game's important semantics drift into UI-only flows.
