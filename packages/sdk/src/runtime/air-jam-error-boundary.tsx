...
import {
  ...
  AIRJAM_RUNTIME_ERROR_REPORT_CONTRACT_VERSION,
...
const createRuntimeReportId = (): string => {
...
> {
  ...
  componentDidCatch(error: Error, errorInfo: ErrorInfo): void {
    const { role, roomId, appId, onError, reportError } = this.props;
    const details = {
      role,
      roomId,
      appId,
    ...
    if (roomId && reportError) {
      try {
        reportError({
          contractVersion: AIRJAM_RUNTIME_ERROR_REPORT_CONTRACT_VERSION,
          reportId: createRuntimeReportId(),
          roomId,
          role,
          code: "AJ_RUNTIME_RENDER_CRASH",
          errorName: /^[A-Za-z][A-Za-z0-9._-]{0,79}$/u.test(error.name)
            ? error.name
            : "Error",
          digest: createErrorDigest(error),
          occurredAt: new Date().toISOString(),
        });
      } catch {
        // Operational reporting must never interfere with the recovery UI.
      }
    }
