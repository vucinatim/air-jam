...
- Confidence: high
- Evidence: `apps/platform/src/server/releases/release-public-url.ts#getHostedReleasesBaseUrl` falls back to `getSiteUrl()`; `apps/platform/src/app/releases/g/[gameId]/r/[releaseId]/generations/[generationId]/[[...assetPath]]/route.ts#GET` serves creator HTML/JS; `apps/platform/src/components/arcade/game-player.tsx#GamePlayer` renders the host iframe without `sandbox`; `controller-game-frame.tsx` combines `allow-scripts` and `allow-same-origin`; `NEXT_PUBLIC_RELEASES_BASE_URL` / `AIRJAM_RELEASES_BASE_URL` are optional.
- Current behavior: Unless a separate base URL is configured and routed correctly, arbitrary creator scripts run at `airjam.io` paths inside an unsandboxed host iframe and are directly navigable. Origin isolation is optional.
