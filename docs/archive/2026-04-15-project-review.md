...
3. Air-capture is a hybrid reference game + SDK stress test. The prefab-preview subsystem (`games/air-capture/src/prefab-preview/` + conditional routing in `src/app.tsx`) is dev tooling leaking into the game and should eventually move into a dedicated internal tool.
...
4. ~~Near-duplicate state-access hooks (`useAirJamHostState`, `useHostSession`). Verify against the ledger, then dedupe whatever remains.~~ Resolved by folding selector support into `useAirJamHost()` / `useAirJamController()` and removing the duplicate public hooks.
...
2. **Extract prefab-preview out of `games/air-capture`** (verified leak at [`games/air-capture/src/app.tsx:7`](../../games/air-capture/src/app.tsx)). Move it into a dedicated internal tool. Don't ship dev tooling inside a shipping game.
