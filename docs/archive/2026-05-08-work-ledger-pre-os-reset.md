...
7. run the final prerelease overpass in [Final Release Checks Plan](./2026-04-19-final-release-checks-plan.md)
...
Canonical prerelease plan: [V1 Release Launch Plan](../plans/v1-release-plan.md)
...
Status: active  
Plan: [V1 Release Launch Plan](../plans/v1-release-plan.md)
...
1. [Final Prerelease Hardening And Cleanup Plan](./2026-04-18-final-prerelease-hardening-and-cleanup-plan.md)
2. [Final Release Checks Plan](./2026-04-19-final-release-checks-plan.md)
3. [V1 Release Launch Plan](../plans/v1-release-plan.md)
4. [Last Band Standing Polish Plan](./2026-04-20-last-band-standing-polish-plan.md)
5. [Prerelease Agent Dev Loop Hardening Plan](./2026-05-02-prerelease-agent-dev-loop-hardening-plan.md)
6. [Landing, Arcade, and Controller Polish Plan](./2026-05-03-landing-arcade-controller-polish-plan.md)
7. [Controller And Platform Settings Ownership Plan](./2026-05-03-controller-platform-settings-ownership-plan.md)
8. [NPM Public Release And Automation Plan](./2026-05-03-npm-public-release-and-automation-plan.md)
9. [Public Package Surface Rationalization Plan](./2026-05-05-public-package-surface-rationalization-plan.md)
10. [On-Demand Full-Stack PR Preview Plan](./2026-05-06-shared-preview-deployment-plan.md)
11. [Railway API Foundation And Agentic OS Plan](./2026-05-07-railway-api-foundation-and-agentic-os-plan.md)
12. [Repo Operating System Reset Plan](./2026-05-07-repo-operating-system-reset-plan.md)

Immediate next work:

1. reset the repo operating system so current truth, history, navigation, and active plan state stop competing for the same surfaces
2. run the final prerelease manual overpass for the launch set and hosted lanes
3. upload/prove the hosted game path through the dashboard
4. finish release media/blogs and the final landing-page overlook
5. merge into `master`, deploy, and run live validation in the canonical release order
6. harden the local agent/dev loop enough that the next external one-shot game test starts from one command, one visible preview surface, and one reset path
7. turn the now-live deployment split into an explicit production topology and env contract, including the dedicated release moderation/browser-runtime lane
8. remove the dashboard-only hosted game creation gap so platform game records can be created and maintained cleanly from the `airjam` CLI before release submission
9. complete the bounded landing/Arcade/controller polish pass, including the Arcade settings interactivity fix, explicit landing featured games, controller chrome tightening, and removal of dead settings controls
10. refactor controller settings ownership so host-owned room settings and controller-local settings become explicit contracts instead of one ambiguous shared surface
11. replace the preview lane's Railway CLI transport with a shared Railway Public API foundation that can later serve the wider agentic operating system through both CLI and MCP adapters
...
6. the late 2026-04-15 doc cleanup pass collapsed all prerelease and release checks into one consolidated [Final Release Checks Plan](./2026-04-19-final-release-checks-plan.md) and archived the Game UI Scaling Plan and Stage 3 Polish Plan now that their implementation tracks are complete
...
19. the pre-preview canonicalization pass is now active: platform deployment/public-origin resolution is centralized behind one typed config boundary, platform auth/session/media consumers are moving onto that contract, the release browser worker now uses an explicit bearer-token gate instead of an implicit public websocket proxy, and the platform bootstrap audit has now kept `NEXT_PUBLIC_AIR_JAM_APP_ID` while dropping `NEXT_PUBLIC_AIR_JAM_HOST_GRANT_ENDPOINT` from the first-party deploy contract
20. the first preview feasibility pass now has repo-owned foundations instead of just notes: the repo is confirmed linked to the live Vercel and Railway projects, the realtime server now has Railway config-as-code at `packages/server/railway.json`, the maintainer CLI can now emit a canonical per-PR preview manifest for future automation, and the first maintainer-triggered GitHub preview workflow entrypoint now exists
...
27. the SDK authoring ergonomics review is captured in [SDK Game Authoring Ergonomics Plan](./2026-04-19-sdk-game-authoring-ergonomics-plan.md), including agreed audio, `SurfaceViewport`, join-controls, Pong helper, and sound-type cleanups plus the open runtime-state decision
28. Last Band Standing's next bounded polish pass is captured in [Last Band Standing Polish Plan](./2026-04-20-last-band-standing-polish-plan.md), covering controller lobby/start/home fixes, host gameplay layout polish, controller game-over scrolling, song buckets, and randomized clip starts
...
30. hosted release serving now injects one explicit `hosted-release` runtime topology into the served HTML bootstrap, and runtime consumers use that same published topology before falling back to looser inference; this removes leaked dev-control traffic from published builds and routes hosted sockets to the configured backend origin instead of accidentally defaulting to the local platform origin
...
38. the public hosted catalog is now fully live on production: all six first-party games have hosted records, live releases, Arcade visibility, source links, and template copy commands, and the legacy app host is now being collapsed onto `airjam.io` as the canonical public domain
39. the on-demand preview lane is now materially live instead of plan-only: the repo owns `preview up/down`, preview DB create/drop, Railway duplicate/deploy/destroy, Vercel deploy/destroy, and create/destroy GitHub workflows; Namecheap wildcard preview DNS is live; Railway production now points at the repo-owned config-as-code paths for both server and worker; Vercel preview Deployment Protection is now explicitly disabled so preview hosts are publicly accessible; a disposable fake-PR proof validated Railway environment duplication, preview schema creation, server deploy, worker deploy, Railway public-domain resolution, Vercel preview-tagged deployments, and a public preview-host alias returning `200`; and the disposable proof environment was then torn back down so provider state returned to a clean `production`-only baseline
39. hosted release bundling now vendors remote font CSS/assets into the artifact at bundle time, so published Arcade releases can keep the strict platform CSP while still supporting Google Fonts-style authoring inputs
40. hosted game media is now a first-class machine/CLI lane: the platform exposes owned-game media inspect/upload/finalize/assign/archive endpoints under `/api/cli/games/[slugOrId]/media`, `@air-jam/devtools-core` exposes typed helpers including one-shot file upload, and `create-airjam` now ships `airjam game media inspect|upload|clear` so thumbnails, covers, and preview videos no longer require dashboard-only setup
41. the bounded landing/Arcade/controller polish pass is now landed locally: landing featured games are explicit and the agent demo now cycles real Air Jam workflow prompts, Arcade hides its visible scrollbar and exposes exact template commands on hover while renaming `Minimal` to `Minimal Template` publicly, preview-controller roster rows are compressed to one line, and controller settings interactivity is no longer blocked by page-wide touch suppression leaking into the shared sheet surface
42. controller settings debugging exposed a deeper ownership gap: the intended product model needs both host-owned room settings and controller-local device settings, but the current controller sheet still mixes those domains without an explicit mirrored host-settings contract; the bounded refactor is now tracked in [Controller And Platform Settings Ownership Plan](./2026-05-03-controller-platform-settings-ownership-plan.md)
43. the npm/package release story is now explicitly split from general prerelease execution: old published npm versions should remain installable but unsupported, the first real public package lane should make plain `npx create-airjam` work from `latest`, and the required package graph / GitHub workflow / deprecation steps are tracked in [NPM Public Release And Automation Plan](./2026-05-03-npm-public-release-and-automation-plan.md)
44. the intended long-term public npm surface is now narrowed further to four products only: `@air-jam/sdk`, `@air-jam/server`, `@air-jam/mcp-server`, and `create-airjam`; the required packaging refactor and the deprecation path for already-published low-level packages are now tracked in [Public Package Surface Rationalization Plan](./2026-05-05-public-package-surface-rationalization-plan.md)
...
Status: completed  
Plan: [Game Structure Alignment Plan](./2026-04-27-game-structure-alignment-plan.md)
...
Plan: [Game UI Scaling Plan (Archived)](./archive/2026-04-15-game-ui-scaling-plan.md)
...
Status: archived 2026-04-15  
Plan: [Stage 3 Polish Plan (Archived)](./archive/2026-04-15-stage-3-polish-plan.md)
...
Status: completed  
Plan: [Controller Preview Workspace Plan](./archive/2026-04-14-controller-preview-workspace-plan.md)
...
Status: completed  
Plan: [Agent Runtime Contract Plan](./archive/2026-04-09-agent-runtime-contract-plan.md)
...
Status: completed  
Plan: [Prerelease Systems Closeout Plan](./archive/2026-04-09-prerelease-systems-closeout-plan.md)
...
Status: completed  
Plan: [Controller Preview Dock Plan](./archive/2026-04-09-controller-preview-dock-plan.md)
...
Status: planned  
Plan: [Final Release Checks Plan](./2026-04-19-final-release-checks-plan.md)
...
Plan: [Hosted Release CLI And MCP Plan](./2026-05-08-hosted-release-cli-and-mcp-plan.md)
...
Plan: [Remote Rooms And Display Surfaces Plan](./2026-04-24-remote-rooms-and-display-surfaces-plan.md)
...
1. [Standard Lifecycle Contract Plan](./archive/2026-04-09-standard-lifecycle-contract-plan.md)
2. [Composition Shell Contract Plan](./archive/2026-04-09-composition-shell-contract-plan.md)
3. [Showcase Games Release Readiness Plan](./archive/2026-04-09-showcase-games-release-readiness-plan.md)
4. [Showcase Games Release Readiness Checklist](./archive/2026-04-09-showcase-games-release-readiness-checklist.md)
5. [Public Release Security Hardening Plan](./archive/2026-04-09-public-release-security-hardening-plan.md)
6. [SDK Extraction Clean-Swap Plan](./archive/2026-04-09-sdk-extraction-clean-swap-plan.md)
7. [Controller Preview Dock Plan](./archive/2026-04-09-controller-preview-dock-plan.md)
8. [Agent Runtime Contract Plan](./archive/2026-04-09-agent-runtime-contract-plan.md)
9. [Prerelease Systems Closeout Plan](./archive/2026-04-09-prerelease-systems-closeout-plan.md)
