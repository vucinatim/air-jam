...
## 7. Monorepo Maintainer Note

If you are working inside the Air Jam monorepo itself rather than a scaffolded game, the repo also includes a dev log viewer:

```bash
pnpm dev:logs
```

That is a repo-maintainer convenience, not a normal requirement for scaffold users.
