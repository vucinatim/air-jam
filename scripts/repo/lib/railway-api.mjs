...
export const RAILWAY_USAGE_SOURCE_VERSION =
  "railway-graphql-v2-usage@2026-08-29";
...
const toMicrousd = (dollars) => Math.round(dollars * 1_000_000);

export const calculateRailwayUsageCost = (measurements) => {
  if (!Array.isArray(measurements)) {
    throw new RailwayApiError("Railway usage measurements must be an array.");
  }
  ...
  return {
    amountMicrousd: Object.values(breakdown).reduce(
      (total, value) => total + value,
      0,
    ),
    breakdownMicrousd: breakdown,
  };
};

const normalizeRailwayUsageMeasurements = ({ entries, valueKey, label }) => {
  if (!Array.isArray(entries)) {
    throw new RailwayApiError(`Railway ${label} usage must be an array.`);
  }
...
export const createRailwayApiClient = ({
  ...
  const getProjectUsageEvidence = async ({
    projectId,
    observedAt = new Date(),
  }) => {
    ...
    if (!billingPeriod?.start || !billingPeriod?.end) {
      throw new RailwayApiError(
        `Railway did not return a billing period for project ${projectId}.`,
      );
    }
    ...
    if (
      ...
      throw new RailwayApiError(
        `Railway returned an invalid billing period for project ${projectId}.`,
      );
    }
    ...
    const actualMeasurements = normalizeRailwayUsageMeasurements({
      entries: data?.usage,
      valueKey: "value",
      label: "actual",
    });
    const projectedMeasurements = normalizeRailwayUsageMeasurements({
      entries: data?.estimatedUsage,
