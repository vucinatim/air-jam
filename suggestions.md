...
4. Replace raw client-sent API keys with short-lived signed host tokens issued by the platform API and validated in one Socket.IO auth middleware.
...
4. **Public SDK surface audit:** Re-audit `packages/sdk/src/index.ts` and drop exports that were added during the Arcade reset but are not part of the clean long-term public API.
