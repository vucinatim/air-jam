...
export function ControllerMenuSheet({
  ...
  const overlayChrome = (
    ...
    >
      ...
      {/* Empty space for the centered notch to sit over. */}
      ...
      <div className="flex min-w-0 flex-1 justify-end gap-2 pl-2">
        {hasControllerCapability ? (
          <Button
            ...
            aria-label={
              hostQrVisible ? "Hide host join QR" : "Show host join QR"
            }
        ...
        {activeUrl && hasControllerCapability ? (
          <Button
            type="button"
            variant="outline"
            size="icon-touch"
  ...
  const closedChrome = (
    <header
      className="pointer-events-none absolute top-0 right-0 left-0 z-50 flex w-full shrink-0 items-center px-3 pt-2 pb-2"
      style={topChromeStyle}
    >
      <ControllerMenuLeadingChrome
        selfProfileForAvatar={selfProfileForAvatar}
                    ...
                    <section className="flex flex-col gap-3">
                      {!hasControllerCapability ? (
                        <p className="text-muted-foreground flex items-start gap-2 rounded-2xl border border-white/10 bg-white/[0.03] px-3 py-2.5 text-xs leading-relaxed">
                          ...
                          <span>
                            Joined with room code. Scan the host QR to use room
                            controls.
                          </span>
                        </p>
                      ) : null}
