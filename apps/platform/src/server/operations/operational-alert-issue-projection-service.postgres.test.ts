...
describeWithPostgres(
  "operational alert issue projection PostgreSQL lifecycle",
  () => {
    ...
    const persistAlertRevision = async ({
      ...
      const evaluation: OperationalSloEvaluationV1 = {
        ...
        environment: "test",
        service: "realtime_server",
        windowStartedAt: new Date(observedAt.getTime() - 60_000).toISOString(),
    ...
    it("keeps permission failure visible, supports audited idempotent requeue, and preserves alert truth", async () => {
      ...
      await persistAlertRevision({ key: alertKey, revision: 2 });
      ...
      await persistAlertRevision({ key: alertKey, revision: 3 });
      await synchronizeNextOperationalAlertIssueProjection({
        database,
        repository,
      });
      ...
      const rateLimited = await runOperationalAlertIssueProjectionCycle({
        database,
        repository,
        workerId: "worker:rate-limited",
        projector: async () => {
          throw new GitHubAlertIssueAdapterError({
            code: "github.http_403",
      ...
      expect(
        Date.parse(rateLimited.projection.availableAt),
      ).toBeGreaterThanOrEqual(retryStartedAt + 59_000);
      await expect(
        requeueOperationalAlertIssueProjection({
          ...input,
    ...
    it("fences concurrent claims and repairs expired leases without duplicate authority", async () => {
      ...
      ).rejects.toThrow(/valid operations identifier/i);
