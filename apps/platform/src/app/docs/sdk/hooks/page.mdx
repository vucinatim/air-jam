...
const HostView = () => {
  ...
  return (
    <div>
      <h1>Room: {roomId}</h1>
      <img src={`https://api.qrserver.com/v1/create-qr-code/?data=${joinUrl}`} />
...
|----------|-----------------|
| Main host component with UI | `useAirJamHost` |
| Individual game objects (ships, projectiles) | `useGetInput` |
| Components that render every frame | `useGetInput` |
| Components that need player list | `useAirJamHost` |
...
const ControllerView = () => {
  ...
  if (connectionStatus === "disconnected") {
    return <div>Disconnected. <button onClick={reconnect}>Retry</button></div>;
  }
