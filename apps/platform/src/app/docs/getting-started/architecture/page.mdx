...
│  ┌────────────────────┐  ┌────────────────────┐  ┌────────────────────────┐ │
│  │   Game Catalog     │  │   Arcade Mode      │  │   Controller Shell     │ │
│  │   Developer Portal │  │   Game Launcher    │  │   (Mobile Wrapper)     │ │
│  └────────────────────┘  └────────────────────┘  └────────────────────────┘ │
└─────────────────────────────────────────────────────────────────────────────┘
                                      │
                                      ▼
...
│  ┌────────────────────┐  ┌────────────────────┐  ┌────────────────────────┐ │
│  │   Room Manager     │  │   Auth Service     │  │   Event Router         │ │
│  │   (State + Rooms)  │  │   (API Keys)       │  │   (Socket.IO)          │ │
│  └────────────────────┘  └────────────────────┘  └────────────────────────┘ │
└─────────────────────────────────────────────────────────────────────────────┘
                    ▲                                       ▲
                    │                                       │
        ...
        │                       │               │                      │
┌───────┴───────┐       ┌───────┴───────┐  ┌────┴────┐         ┌───────┴───────┐
│  Your Game    │       │  Your Game    │  │ Phone 1 │         │    Phone N    │
│  (Host)       │       │  (Host)       │  │ Ctrl    │   ...   │    Ctrl       │
│               │       │               │  │         │         │               │
│  @air-jam/sdk │       │  @air-jam/sdk │  │  SDK    │         │    SDK        │
└───────────────┘       └───────────────┘  └─────────┘         └───────────────┘
```
...
|---------|---------------|
| **Room Manager** | Creates/destroys rooms, tracks connected players |
| **Auth Service** | Validates API keys, enforces rate limits |
| **Event Router** | Routes input/signals between hosts and controllers |
| **Focus System** | Controls whether system or game receives input |

**Socket Events:**

| Event | Direction | Purpose |
|-------|-----------|---------|
| `host:register` | Host → Server | Create/join a room |
| `controller:join` | Controller → Server | Join existing room |
| `controller:input` | Controller → Server → Host | Send input to game |
| `host:signal` | Host → Server → Controller | Send haptics/toasts |
| `server:state` | Server → All | Broadcast game state |
...
└─────────────────────────────────────────────────────────┘
         ...
         ▼                 ▼                 ▼
┌─────────────────┐ ┌─────────────┐ ┌───────────────────┐
│ useAirJamHost   │ │ useGetInput │ │ useAirJamController│
│ (Full API)      │ │ (Perf)      │ │ (Mobile)          │
└─────────────────┘ └─────────────┘ └───────────────────┘
```
