...
interface RocketModelProps {
  /** Whether to show exhaust particles */
  showParticles?: boolean;
  /** Whether the rocket is oriented horizontally (for flying rockets) or vertically (for preview) */
  horizontal?: boolean;
}

/**
 * Reusable rocket model component
 * Can be used in both preview and actual game rockets
 */
export function RocketModel({
  ...
  useFrame((_state, delta) => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      const resetThreshold = horizontal ? -3 : -3;
      if (lifetimes[i] <= 0 || positions[i3 + 1] < resetThreshold) {
  ...
  // When horizontal, rotate -90° around X axis to point along -Z (matching Rocket component's expected forward direction)
  const groupRotation: [number, number, number] = horizontal
    ? [-Math.PI / 2, 0, 0]
    : [0, 0, 0];
